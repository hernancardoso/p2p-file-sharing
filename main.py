...
def main():
    ...
    threading.Thread(
        target=anuncios.init
    ).start()  # start thread for UDP send/receive initializer
    threading.Thread(
        target=descargas.init
    ).start()  # start thread for TCP download request/response initializer
        ...
        while read != "CLOSED" or read != "EXIT":
            ...
            if args[0] == "list":
                ...
                for file in v.availableFiles:
                    v.availableFiles[file]["servers"] = {
                        k: v
                        for (k, v) in v.availableFiles[file]["servers"].items()
                        if timeNow - v <= 90000
                    }
                    if len(v.availableFiles[file]["servers"]) == 0:
                        ...
                        response += (
                            file
                            + "\t"
                            + v.availableFiles[file]["fileSize"]
                            + "\t"
                            + str(v.availableFiles[file]["fileNames"])
                            + "\n\r"
                        )
                ...
                if args[1] not in v.availableFiles:
                    ...
                    if v.errorDownloading == "":
                        ...
                        response = (
                            "El archivo "
                            + args[0]
                            + " fue descargado a un promedio de "
                            + str(int(fileSize / ((endTime - startTime) / 1000)))
                            + " Bps"
                        )
                    else:
