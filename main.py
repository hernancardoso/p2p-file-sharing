...
def main():
    ...
    threading.Thread(target=anuncios.init).start() #start thread for UDP send/receive initializer
    threading.Thread(target=descargas.init).start() #start thread for TCP download request/response initializer
...
def telnetReceiver():
    ...
    while True:
        ...
        while read != "CLOSED" or read != "EXIT":
            ...
            if(args[0] == "list"):
                ...
                for file in v.availableFiles:
                    v.availableFiles[file]["servers"] = {k:v for (k,v) in v.availableFiles[file]["servers"].items() if timeNow - v <= 90000}
                    if(len(v.availableFiles[file]["servers"]) == 0):
                        v.availableFiles = {k:v for (k,v) in v.availableFiles.items() if k != file}
                    else:
                        response += file + "\t" + v.availableFiles[file]["fileSize"]  + "\t" + str(v.availableFiles[file]["fileNames"]) + "\n\r"
                ...
                if(os.path.isfile(path + "\\" +file)) :
                    v.myFiles[utils.md5(path + "\\" +file)] = {"fileName": file, "fileSize": utils.getFileSize(path + "\\" +file)}
                ...
                for file in v.myFiles:
                    response += file + "\t" + v.myFiles[file]["fileSize"]  + "\t" + v.myFiles[file]["fileName"] + "\n\r"
                if (len(v.myFiles) == 0) :
                ...
                if(args[1] not in v.availableFiles):
                    ...
                    v.errorDownloading = ""
                    x = threading.Thread(target=descargas.startDownload, args=(args[1],))
                    x.start()
                    x.join()
                    if(v.errorDownloading == ""):
                        ...
                        path = config.data["shared_folder"]
                        v.myFiles[utils.md5(path + "\\" +fileName)] = {"fileName": fileName, "fileSize": utils.getFileSize(path + "\\" + fileName)}
                        response = "El archivo " + args[0] + " fue descargado a un promedio de " + str(int(fileSize / ((endTime - startTime)/1000))) + " Bps"
                    else : 
        ...
        connectionSock.shutdown(SHUT_RDWR)
