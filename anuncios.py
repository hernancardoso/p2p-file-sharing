...
def init():
    ...
    threading.Thread(target=periodicAnnounce).start() #start thread to send ANNOUNCES each 30 seconds
...
def createAnnounce(startPosition):
    ...
    for i in range(startPosition, min(int(config.data["max_announce_lines"]) + startPosition, len(v.myFiles))):
        ...
        tempMessage = myFiles[i][1] + "\t" + utils.getFileSize(filePath) + "\t" + utils.md5(filePath) + "\n" 
...
def listen():
    ...
    while(True):
        ...
        if(address[0] != config.data["ip"]): #Ignore self datagrams
            ...
            if(config.data["debugging"]): print("\nNew UDP message\n" + str(read))
            ...
            if(read.find("ANNOUNCE") != -1):
                ...
                for line in read.split("\n"):
                    ...
                    if(fileMD5 not in v.availableFiles):
                        #new file arrived
                        v.availableFiles[fileMD5] = {"fileNames": [fileName], "fileSize": fileSize, "servers": {address[0] : timestamp}}
                    else:
