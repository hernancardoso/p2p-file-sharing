...
def serveClient(clientSock, addr):
    ...
    if(read == "DOWNLOAD"):
        ...
        if (not md5Code in v.myFiles):
            #client doesnt have requested file in shared_folder
            clientSock.sendall("DOWNLOAD FAILURE\nMISSING\n".encode())
            return False

        if (not reqSize.isnumeric() or not reqStart.isnumeric() or int(reqStart) > int(v.myFiles[md5Code]["fileSize"])):
            clientSock.sendall("DOWNLOAD FAILURE\nBAD REQUEST\n".encode())
            print("error 1")
            return False
        ...
        f = open(config.data["shared_folder"] + "\\" + v.myFiles[md5Code]["fileName"], "rb")
        f.seek(reqStart)
...
def startDownload(md5Code):
    ...
    v.availableFiles[md5Code]["servers"] = {k:v for (k,v) in v.availableFiles[md5Code]["servers"].items() if timeNow - v <= 90000}
    ...
    availableServers = v.availableFiles[md5Code]["servers"] #shortcut for availableServers
    
    totalServers = len(availableServers)

    actualServer = 0
    chunkSize = int(fileSize / totalServers) #The chunks size is divided by the number of available server
...
def downloadChunk(ip, md5Code, reqByte, reqSize):
    ...
    if(answer[:12].decode().find("DOWNLOAD OK\n") != -1):            
        ...
        utils.println("Got "+str(len(answer)) + " - "+str(downloadedSize) + "/" + str(reqSize)  + " ("+ str(int(downloadedSize * 100 / reqSize)) + ")%")
        f.write(answer)
